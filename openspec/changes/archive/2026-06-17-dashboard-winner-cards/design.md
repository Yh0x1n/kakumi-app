...
1. **Query categories + tournament name**:

   ```sql
   SELECT tc.*, t.name AS tournament_name
...
**Kata informal** — use the composite index `ix_kata_informal_performances_category_athlete`:
...
**Score → string**: `str(score)` — for integers gives `"3"`, for floats `"24.5"`.
...
`kakumi_app/states/dashboard_state.py` — new file, following the `ResultsState` patterns (thin state, service call in try/except).
...
- No `error_message`/`empty_message` vars — empty state is handled in the template with `rx.cond`.
- `is_loading` follows existing pattern, available for future loading spinners.
...
Add to existing `from .states.*` import block (alphabetical):
...
**Before** — 4 static placeholder cards with `rx.Var.range(4)`:
...
rx.grid(
    rx.foreach(
        ...
        lambda i: rx.card(
            rx.link(
                rx.text(f"Resultado {i + 1}", weight="bold", font_size="10"),
                underline="none", height="100%",
            ),
            border_width="thick", border_radius="1em",
        ),
    ),
    columns="2", spacing="4", width="50%", padding="0.5em",
),
```

**After** — live cards with empty state via `rx.cond`:
...
rx.cond(
        ...
        rx.foreach(
            ...
            lambda card: rx.card(
                rx.vstack(
                    ...
                    align="center", spacing="1", padding="0.5em",
                ),
                border_width="thick", border_radius="1em", width="100%",
            ),
        ),
        columns="2", spacing="4", width="50%", padding="0.5em",
    ),
    rx.card(
        ...
        border_width="thick", border_radius="1em", padding="1em",
    ),
),
```
...
Change `/home` route from:

```python
app.add_page(
    dashboard, route="/home", title="Kakumi Tournament Manager",
...
This guarantees auth guard fires first. If unauthenticated, `check_auth_redirect` redirects before `load_recent_winners` runs.
...
Browser loads /home
  → Reflex runs on_load chain:
    1. AuthState.check_auth_redirect()
       → redirect to /login if not authenticated
    2. DashboardState.load_recent_winners()
       → ResultsService.get_recent_winners()
         → SQL: COMPLETED + first_place_id IS NOT NULL, ORDER BY id DESC LIMIT 4
         → Bulk-load athlete names
         → Score resolution per modality
         → Return list[dict] capped at 4
       → DashboardState.winner_cards = result
  → Template re-renders:
    - winner_cards non-empty → grid of up to 4 cards
    - winner_cards empty → single "Sin resultados aún" card
```
...
| 6 completed categories | Only 4 newest (ordered by category id DESC) |
| Kata informal with no performances row | Score shows 0 |
| Team modality winner | Score shows 0 |
| Database error on load | winner_cards stays [] → empty-state card |
...
1. **Empty DB** → `[]`
2. **Single kumite category** → 1 card with correct winner name, match score, category/tournament name
3. **Single kata informal category** → 1 card with `final_score` from `KataInformalPerformance`
4. **Multiple categories, some incomplete** → only COMPLETED with first_place_id appear
5. **6 completed categories** → only 4 returned, ordered by id DESC
6. **Kata elimination (not informal)** → score from match aka_score/ao_score
7. **Team modality** → score 0

### Integration

- Assert `on_load` chain order is `[AuthState.check_auth_redirect, DashboardState.load_recent_winners]`
- Assert `rx.cond` renders empty card when winner_cards empty
...
Single commit — the feature is atomic and small (3 files).
