# Calendar Behavior Specification
...
Define the behavior of the calendar date picker component (`date_calendar_popover`) so its interactive elements (day buttons, navigation arrows, trigger button) do NOT trigger `type="submit"` behavior when used inside an `rx.form()` context. Only the explicit "Save" submit button in the hosting form SHALL trigger submission.
