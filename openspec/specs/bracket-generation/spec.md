# Bracket Generation Specification
...
Define how bracket generation (`_generate_brackets_for_tournament()`) handles categories based on their `kata_flow_mode`. INFORMAL categories use the `KataInformalPerformance` model for operations-driven scoring and MUST NOT produce bracket structures. Only STANDARD categories generate Match records.
